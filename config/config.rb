...
# Inject database password from .env
AppConfig[:db_url] = "jdbc:mysql://db:3306/archivesspace?useUnicode=true&characterEncoding=UTF-8&user=as&password=#{ENV['MYSQL_PASSWORD']}&useSSL=false&allowPublicKeyRetrieval=true"

# Configuration defaults are shown below
AppConfig[:authentication_sources] = [{
                                         :model => 'LDAPAuth',
                                         :hostname => 'nis.cc.lehigh.edu',
#                                         :port => 636,
                                         :port => 389,
                                         :base_dn => 'dc=lehigh,dc=edu',
                                         :username_attribute => 'uid',
                                        :attribute_map => {:cn => :name},
# only required for port 636
...
# Configure PUI contact settings
AppConfig[:global_email_from_address] = "inspec@lehigh.edu"
AppConfig[:pui_email_enabled] = true
AppConfig[:pui_email_override] = 'inspec@lehigh.edu'
AppConfig[:pui_request_email_fallback_to_address] = 'inspc@lehigh.edu'
AppConfig[:pui_request_email_fallback_from_address] = 'inspc@lehigh.edu'
...
# Resequesnce tree notes on startup?
